...
def evaluate(model, loader):
    ...
    with torch.no_grad():
        for audio, text, label, text_len in loader:
            text = text.to(DEVICE)
            out = model(text)        # ← 不傳 lengths，讓 LSTM 跑完整 500 timestep   
...
def train(seed=42):
    ...
    criterion = nn.CrossEntropyLoss()   # 拿掉 weight)
        

    optimizer = torch.optim.Adam(model.parameters(), lr=LR, eps=1e-7)
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(  # ← 放在這裡，只初始化一次
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if ua > best_ua:
            ...
            torch.save(model.state_dict(), f'best_tbl_seed{seed}.pt')
            marker = '  ← best'
