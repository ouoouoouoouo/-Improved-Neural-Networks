# train_scnn.py  ── S-CNN（論文 baseline，無 weight_decay）
import torch
...
def train(seed=42):
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=LR, eps=1e-7)  # 無 weight_decay
    ...
    print(f"\n{'='*60}")
    print(f"  [S-CNN]  Seed={seed}  Epochs={EPOCHS}  LR={LR}  WD=0")
    print(f"{'='*60}")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if ua > best_ua:
            ...
            torch.save(model.state_dict(), f'best_scnn_seed{seed}.pt')
            marker = '  ← best'
...
if __name__ == '__main__':
    ...
    print(f"\n{'='*60}")
    print(f"  [S-CNN]  3-run average  WA={sum(was)/3:.2f}%  UA={sum(uas)/3:.2f}%")
